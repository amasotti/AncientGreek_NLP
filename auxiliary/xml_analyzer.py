                            ...
                            for other_tokens in infos.keys():
                                try:
                                    if infos[other_tokens]['head'] == head_nr and (infos[other_tokens]['relation'] in  ['SBJ','SBJ_CO']):
                                        single_verb_dictionary['subjects'].append(infos[other_tokens]['lemma'])
                                    if infos[other_tokens]['head'] == head_nr and (infos[other_tokens]['relation'] in ['OBJ','OBJ_CO']):
                                        single_verb_dictionary['objects'].append(infos[other_tokens]['lemma'])
...
def to_counter(data_df):
    ...
    for _, infoList in data_df.iterrows():
        ...
        if not pd.isna(infoList.values[1]):
            verb['subjects'] = collections.Counter(infoList.values[1].split(','))
        if not pd.isna(infoList.values[2]):
            verb['objects'] = collections.Counter(infoList.values[2].split(','))
        verbs.append(verb)
    return verbs
...
# splitted_dictionary(verb_dictionary=verbs_splitted,xml_data=verbi,save=True,output_fp='../data/verb_data_labelled_new.json')
...
#subjObjdata = extend_findings(preliminary_data=subjObjdata,new_source=args.odyssee_raw,verb_list=vocabulary,greedy=True)
...
#sub, obj = holistic_overview(info_counted,save=True,out_path=args.Homer_Media_counted_functions)


## REPEAT FOR THE ACTIVE VERBS FOR COMPARISON
# Extract actives
vocab = extract_diathesis(data=args.verbs_labelled,diathesis='oppositive') # 2062 verbs
body = parse(args.ilias_parsed)
subjObj_actives = subj_obj_extractor(body=body,verbs=vocab,greedy=True)
subjObj_actives = extend_findings(subjObj_actives,args.odyssee_raw,verb_list=vocab,greedy=True)

# save actives
save_extracted_data(subjObj_actives,output_fp=args.extracted_gesamt_info_csv)
...
subj_act, obj_act = holistic_overview(info_actives_counted,save=True,out_path=args.Homer_active_counted_functions)
