...
def splitted_dictionary(verb_dictionary,xml_data,labels = ['mp','oppositive'],save=True,output_fp='data/verb_data_labelled.json'):
    ...
    for label in labels:
        for verb in verb_dictionary[label]:
            forme = extract_forms(verb_list = [verb],data_from_xml=xml_data,save=False)
            final_dictionary[verb] = {"diathesis" : label, "v_forms" : list(forme[verb])}
    if save:
    ...
    if save:
        try:
            with open(out_json,'w',encoding='utf-8') as jsonDump:
                dump = {lemma : ",".join(_ for _ in verb_dict[lemma]) for lemma in verb_dict.keys()}
...
class Vectorize(object):
    ...
    def vectorize_simple(self,verb,target,window=9):
        ...
        context = self.extract_common_neighbours(target=target,verb=verb,window=window)
